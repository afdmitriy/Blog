...
// import { RoutesList } from '../../common/routes/routes.list';
...
//       authQueryRepository.getUserByConfirmationCode = jest
//          .fn()
//          .mockImplementation((confirmationCode: string) => {
//             return user;
//          });
//       authCommandRepository.confirmUser = jest
//          .fn()
//          .mockImplementation((confirmationCode: string) => {
//             return true;
//          });

//       const result = await confirmUserEmailUseCase(
//          user.confirmationData.confirmationCode
//       );

//       expect(result.status).toBe(ResultToRouterStatus.SUCCESS);
//       expect(
//          authQueryRepository.getUserByConfirmationCode
//       ).toHaveBeenCalledTimes(1);
//       expect(authCommandRepository.confirmUser).toHaveBeenCalledTimes(1);
//       expect(
//          authQueryRepository.getUserByConfirmationCode
//       ).toHaveBeenCalledWith(user.confirmationData.confirmationCode);
//       expect(authCommandRepository.confirmUser).toHaveBeenCalledWith(
//          user.confirmationData.confirmationCode
//       );
//    });
...
//       expect(
//          authQueryRepository.getUserByConfirmationCode
//       ).toHaveBeenCalledWith(wrongConfirmationCode);
//    });
...
//       expect(result.data).toStrictEqual(
//          errorMessagesHandleService({
//             message: 'Registration was already confirmed',
//             field: 'code',
//          })
//       );
//       expect(
...
//       const resendUserConfirmationCodeUseCase =
//          authService.resendConfirmationCode;
//       const user = createTestUserFromDb();

//       emailManager.resendRegistrationEmail = jest
//          .fn()
//          .mockImplementation((email: string, confirmationCode: string) => {
//             return 'Email send success';
//          });
//       authQueryRepository.getUserByLoginOrEmail = jest
//          .fn()
//          .mockImplementation((email: string) => {
//             return user;
//          });

//       const result = await resendUserConfirmationCodeUseCase(
//          user.userData.email
//       );

//       expect(result.status).toBe(ResultToRouterStatus.SUCCESS);
//       expect(emailManager.resendRegistrationEmail).toHaveBeenCalledTimes(1);
//       expect(authQueryRepository.getUserByLoginOrEmail).toHaveBeenCalledTimes(
//          1
//       );
//       expect(emailManager.resendRegistrationEmail).toHaveBeenCalledWith(
//          user.userData.email,
//          expect.any(String)
//       );
//       expect(authQueryRepository.getUserByLoginOrEmail).toHaveBeenCalledWith(
//          user.userData.email
//       );
//    });
// });
